...
const DynamicPages = () => {
  ...
  const determineParameters = () => {
    ...
    if (params.gender && !params.category) {
      ...
      if (
        knownGenders.includes(param) ||
        knownGenders.includes(param.replace("s", ""))
      ) {
  ...
  useEffect(() => {
    if (initialLoadComplete) {
      ...
      const { gender: newGender, category: newCategory } =
        determineParameters();
  ...
  const categoriesMatch = (category1, category2) => {
    ...
    if (
      camelToSpace(category1) === normcategory2 ||
      camelToSpace(category2) === normcategory1
    )
      return true;
  ...
  const resetFilters = () => {
    ...
    const { gender: currentGender, category: currentCategory } =
      determineParameters();
                ...
                <div className="active-filters-list">
                  {filters.gender && manuallyAppliedFilters.gender && (
                    <span className="active-filter-tag">
                      gender:{" "}
                      {formatFilterDisplayText("gender", filters.gender)}
                      <button
                  ...
                  {filters.category && manuallyAppliedFilters.category && (
                    <span className="active-filter-tag">
                      category:{" "}
                      {formatFilterDisplayText("category", filters.category)}
                      <button
                        className="remove-filter"
